...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `${memberUser?.name} ${memberUser?.surname} - Top 20`,
    description: `View ${memberUser?.name} ${memberUser?.surname}'s Top 20`,
  }
}

export default async function MemberTop20Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  ...
  const [top20Data, top40CountData, specialRequestsData, successStoriesData] = await Promise.all([
    payload.find({
      collection: 'top20',
